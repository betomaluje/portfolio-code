...
{
    public class TestInputDialogRenderer : DialogRenderer
    {
            ...
            if (dialog.choices.Count > 1)
            {
                ...
                foreach (var choice in dialog.choices)
                {
                    ...
                    button.onClick.AddListener(() =>
                    {
                        ...
                        DialogManager.Instance.MakeChoice(dialog.guid, lastChoice);
                    });

                    // hack to make the EventSystem think it has a first selected game object.
                    // since it's dynamic, we need to fake it this way.
                    // in here, we add it to the EventSystem and also we focus it
                    if (i == 0)
                    {
                        EventSystem.current.firstSelectedGameObject = choiceButton;
                        button.Select();
                    }
                    i++;
                }
