...
namespace Weapons {
    public class BossBeamWeapon : BaseSpawnWeapon {
        ...
        private void StartMoving() {
            _audioSource.Play();
            NonPersistentServiceLocator.Get<PostProcessingManager>().SetProfile("Beam Boss 2").Forget();
            NonPersistentServiceLocator.Get<CinemachineCameraShake>().ShakeCamera(transform, 10, _lifeTime, true);
        }

        private void OnDestroy() {
            NonPersistentServiceLocator.Get<PostProcessingManager>().Reset();
        }
