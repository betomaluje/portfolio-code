...
namespace Weapons {
    ...
    public class FullscreenBossWeapon : MonoBehaviour {
        ...
        private async void PerformShooting() {
            _audioSource.Play();
            if (!string.IsNullOrEmpty(_fullscreenWeaponConfig.PostProcessingProfile)) {
                await NonPersistentServiceLocator.Get<PostProcessingManager>().SetProfile(_fullscreenWeaponConfig.PostProcessingProfile);
            }
            NonPersistentServiceLocator.Get<CinemachineCameraShake>().ShakeCamera(transform, _fullscreenWeaponConfig.ScreenShakeMagnitude, _fullscreenWeaponConfig.ScreenShakeDuration, true);
