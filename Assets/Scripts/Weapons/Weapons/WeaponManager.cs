...
namespace Weapons {
    public class WeaponManager : MonoBehaviour, IWeaponManager {
        ...
        public bool IsCharging => Weapon.AttackType == AttackType.Throwable || Weapon is ICharge;
        ...
        private readonly Dictionary<string, Transform> _weaponTransforms = new();

        private void Awake() {
            _playerBattleInput = GetComponent<PlayerBattleInput>();
            _playerStateMachine = GetComponent<PlayerStateMachine>();

            if (_weaponAnimator != null && _weaponAnimationConfig != null) {
        ...
        public void OverrideWeapons(List<Weapon> weapons, int lastSelectedWeapon) {
            ...
            foreach (var weapon in weapons) {
                TryAddToWeaponTransform(weapon);
            }
        ...
        private void TryAddToWeaponTransform(Weapon weapon) {
            string prefabNameToEnable = weapon.PrefabNameToEnable;

            if (_weaponTransforms.ContainsKey(prefabNameToEnable)) {
                return;
            }
        ...
        private void SetAimCharging(Weapon weapon) {
            bool shouldCharge = weapon.AttackType == AttackType.Throwable || weapon is ICharge;
            PlayerAim.SetShouldChargeWeapon(shouldCharge, weapon is ICharge chargeWeapon ? chargeWeapon.ChargeTime : 0f);
        }
        ...
        private void UpdateWeaponStats() {
            ...
            AnalyticsEvents.OnWeaponSwitched?.Invoke(weapon.Name);
        ...
        private void OnDrawGizmosSelected() {
            PlayerAim?.OnDrawGizmosSelected();
        }
