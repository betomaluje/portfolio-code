...
namespace Modifiers.Powerups {
    public class PowerupHolder : ModifierHolder<PowerupConfig> {
        ...
        protected override void HandleTargetDetected(Transform target) {
            if (target.TryGetComponent<CharacterPowerup>(out var characterPowerup)) {
                // at least one
                var powerupPerformed = false;
                foreach (var config in _itemsConfigs) {
                    if (characterPowerup.DoPowerup(config, target)) {
                        if (!powerupPerformed) {
